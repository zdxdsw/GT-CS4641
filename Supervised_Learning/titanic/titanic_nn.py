...
def main():
    ...
    model = Sequential()
    model.add(Dense(output_dim=2, input_dim=11, init='random_uniform', activation='sigmoid'))
    #model.add(Dropout(0.1))
    model.add(Dense(2, init='random_uniform', activation='relu'))
    ...
    test_df.to_csv('nn_submission.csv', columns=header, index=False, header=True)
